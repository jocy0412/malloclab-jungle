...
#define PRED_FREEP(bp)  (*(void**)(bp))             // 이전 블록의 bp 리턴
#define SUCC_FREEP(bp)  (*(void**)(bp + WSIZE))     // 이후 블록의 bp 리턴
...
int mm_init(void)
{
    ...
    /* padding, prol_header, prol_footer, PRED, SUCC, epil_header */
    ...
    if (extend_heap(CHUNKSIZE / WSIZE) == NULL) // 생성 실패시 -1 리턴
        return -1;
...
static void* extend_heap(size_t words){ // 워드 단위로 받는다.
    ...
    PUT(HDRP(bp), PACK(size, 0));  // header에 size만큼 가용하다는 정보 삽입
    PUT(FTRP(bp), PACK(size, 0));  // footer에 size만큼 가용하다는 정보 삽입
    PUT(HDRP(NEXT_BLKP(bp)), PACK(0, 1));  // 새 에필로그 헤더 갱신

    /* 만약 인접 블록이 가용 블록이라면 연결시킨다. */
    return coalesce(bp);
}
...
void *mm_malloc(size_t size)
{
    ...
    // 요청 사이즈에 header와 footer를 위한 dword 공간(SIZE_T_SIZE)을 추가한 후 align
    asize = ALIGN(size + SIZE_T_SIZE);

    // 할당할 가용 리스트를 찾고(find_fit), 필요하다면 분할해서 할당(place)
    if ((bp = find_fit(asize)) != NULL){  // first fit으로 서칭
        place(bp, asize);  // 필요하다면 분할하여 할당
        return bp;
    }

    // 만약 맞는 크기의 가용 블록이 없다면 새로 힙을 늘려서 새 힙에 메모리를 할당
    extendsize = MAX(asize, CHUNKSIZE);  // 기본 힙 확장 사이즈는 chuncksize이므로 비교 후에 둘 중 더 큰 값으로 사이즈를 정함
...
static void* coalesce(void* bp){
    ...
    // case 1 : 직전, 직후 블록이 모두 할당 -> 해당 블록만 free list에 삽입
    ...
    // 인접 블록과 연결된 새 가용 블록을 free list에 추가
    putFreeBlock(bp);
...
/*
    first_fit : 힙의 맨 처음부터 탐색하여 요구하는 메모리 공간보다 큰 가용 블록의 주소를 반환
*/
static void* find_fit(size_t asize){
    /* First-fit */
    void* bp;

    // free list의 맨 뒤(마지막)는 프롤로그 블록이다. Free list에서 유일하게 할당된(GET_ALLOC=1) 블록이므로 만나면 탐색 종료
...
/*
    place(bp, size)
    : 요구 메모리를 할당할 수 있는 가용 블록을 할당. 이 때 분할이 가능하면 분할
*/
...
/*
    putFreeBlock(bp) : 새로 반환되거나 생성된 가용 블록을 free list의 첫 부분에 삽입
*/
...
/*
    removeBlock(bp) : 할당되거나 연결되는 가용 블록을 free list에서 떼어냄
*/
void removeBlock(void *bp){
    // free list의 첫번째 블록을 제거할 때
    if (bp == free_listp){
        PRED_FREEP(SUCC_FREEP(bp)) = NULL;
        free_listp = SUCC_FREEP(bp);
    }
    // free list의 임의의 블록을 제거할 때
    else{
...
void mm_free(void *bp)
{
    // free가 필요한 블록의 size를 확인
    size_t size = GET_SIZE(HDRP(bp));

    // header와 footer에 확인한 size만큼 가용하다는 정보를 삽입
    PUT(HDRP(bp), PACK(size, 0));
    PUT(FTRP(bp), PACK(size, 0));

    // 만약 인접 블록이 가용 상태라면 연결
    coalesce(bp);
}
...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    // 원래 메모리 크기(copySize)보다 적은 크기(size)에 대해 재할당 요청이 오면 요청한 크기 만큼의 메모리만 할당
    if (size < copySize)
      copySize = size;

    memcpy(newptr, oldptr, copySize);  // newptr에 oldptr를 시작으로 copySize만큼의 메모리 값을 복사한다.
    mm_free(oldptr);  // 기존에 힙에 할당된 메모리는 반환
    return newptr;
}